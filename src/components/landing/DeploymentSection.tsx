    ...
    <div className="section-container">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {useCases.map(({ title, description, image, Icon }) => (
          <article
            key={title}
            className="group relative overflow-hidden rounded-lg border border-border bg-card aspect-[4/5] cursor-pointer transition-all duration-500 hover:shadow-xl hover:-translate-y-1"
          >
            <img
              src={image}
              alt={title}
